...
function App() {
  const { user, theme, setTheme, loading, lang } = useMedwise()
      ...
      <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'dark bg-deep-green text-gray-100' : 'bg-gray-50/50 text-gray-900'}`}>
        {loading ? (
          <div className="min-h-screen flex flex-col items-center justify-center space-y-4">
            ...
            <p className="font-medium animate-pulse">{lang({en:'Initializing MedWise...', pidgin:'Opening MedWise...', ha:'Ana buɗe MedWise...', yo:'Ìfihàn MedWise...', ig:'Na-emeghe MedWise...'})}</p>
          </div>
        ) : !user ? (
          <Auth />
        ) : (
